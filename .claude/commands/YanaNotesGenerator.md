...
2. **UNIFORM STRUCTURE.** Same hierarchy every week: numbered sections per lecture topic, subsections per concept. Heading style comes from `uninotes.sty` only; never define local styling. Document order is fixed: title block, one page contents, lecture content in delivery order (including any worked example the lecture itself walks through, left exactly where the lecture puts it), Key Summary, Worked Examples Q&A (worksheet/solutions/revision examples and anything not tied to one lecture point ONLY), Revision Questions and Answers.
3. **EVERY EQUATION outside worked examples** is displayed inside a red `eqnbox`, centred in a proper math environment, with its `varlist` INSIDE the same box directly under it (first use only), naming each symbol as `Term: FullName (UNITS)` via `\vitem{symbol}{full name}{units}` (or `\vitemnd` for dimensionless). Equations inside `\begin{examplebox}` worked examples are exempt, they stay plain `align`/`equation`, no eqnbox. **Definition and equation, pick both when both exist:** if the source states a written definition for the concept as well as giving its formula, write the prose definition in a `defbox` AND the formula plus varlist in an `eqnbox` right after it, never only one when the source has both. If the source gives only the formula with no written definition, derive a one sentence definition ("Aspect Ratio is the ratio of...") and open the `eqnbox` with that sentence above the equation, rather than fabricating a separate defbox with no source content to back it.
4. **WORKED EXAMPLES IN Q&A FORMAT** (from lecture slides AND worksheets/solutions): inside `\begin{examplebox}{Worked Example: Topic}`, first `\question{...}` stating the problem exactly as given, then `\wstep{X}{plain English explanation of what is done and why}` before each block of maths, mimicking the official solution's method so the steps can be copied directly, ending with `\result{...}` giving the value and its physical meaning. Steps must be simple, logical, and followable. Place it INLINE in the lecture content at the point the lecture presents it if that's where the source has it; only route it to the trailing Worked Examples Q&A section if it comes from a separate worksheet/solutions/revision source, or isn't tied to one specific lecture point.
...
7. **BOX TITLES ALWAYS BRACED AND TITLE CASE:** `\begin{keybox}{Key Result: Gain, Phase and Bandwidth}` — the brace protects commas from tcolorbox key parsing (a past compile failure), and the descriptive phrase after the colon uses Title Case (capitalise major words; keep articles, "and", "of", "the", short prepositions lowercase unless first word), never lowercase ("Key Result: dependent suspension coupling" is wrong).
8. **CONTENTS PAGE, HARD RULE.** Always fits on exactly one page and is always plain black and white. Always call `\notestoc` (never bare `\tableofcontents`, the template already does this): it forces every entry and page number black, since hyperref repatches `\tableofcontents` at `\begin{document}` and would silently recolour it otherwise. If default sizing overflows, add `\tightertoc` on the line before `\notestoc` in that week's `.tex`; if it still overflows, also add `\setcounter{tocdepth}{1}` (sections only).
9. **WRITE TO THE CORRECT PATH FIRST TIME.** All outputs go in the week folder using absolute paths. Verify the folder before writing.
...
**A5. Master markdown.** Synthesise `MODULE_WeekNN_Notes.md` from the extraction files (do not re-read images): full lecture content in delivery order obeying every writing rule (worked examples the lecture walks through stay inline at that point, eqnbox+varlist together for equations outside worked examples, defbox/derived definition logic per rule 3, Title Case box titles), then Key Summary, Worked Examples Q&A (worksheet/solutions/revision examples and anything not tied to one lecture point only), Revision Questions and Answers (every revision question answered fully, tutor style, numerical ones in worked example format). Equations in LaTeX math with their variable lists. Reference cropped figures by filename. This file is the permanent recall source for tutor mode.
...
```
Fix any errors (check the .log), rerun until clean. Confirm the ToC fits on one page and renders black and white (hard rule, rule 8, via `\notestoc`): if it does not fit at default sizing, add `\tightertoc` before `\notestoc` and recompile; if it still does not fit, also add `\setcounter{tocdepth}{1}` (sections only). Report final page count and any warnings.
